...
Public Class IniFile
    ...
    Public Sub New()
        ...
        m_sections = New Dictionary(Of String, IniSection)(StringComparer.InvariantCultureIgnoreCase)

    End Sub

    ' Loads the Reads the data in the ini file into the IniFile object
    Public Sub Load(ByVal sFileName As String, ByVal TextEncoding As Encoding, Optional ByVal bMerge As Boolean = False)
        If Not bMerge Then
            RemoveAllSections()
        End If
    ...
    Public Function AddSection(ByVal sSection As String) As IniSection
        ...
        If m_sections.ContainsKey(sSection) Then
            ...
            '  Trace.WriteLine("Section " & sSection & " already in list: " & m_sections.Count)
        Else
            s = New IniSection(Me, sSection)
            m_sections(sSection) = s
            ' Trace.WriteLine("New Section " & sSection & ": " & m_sections.Count)
        End If
        Return s
    End Function
    ...
    Public Function GetKeyValue(ByVal sSection As String, ByVal sKey As String, Optional ByVal DefaultValue As String = "") As String
    ...
    Public Function SetKeyValue(ByVal sSection As String, ByVal sKey As String, ByVal sValue As String, Optional ByVal DefaultValue As String = "") As Boolean
        Dim s As IniSection = AddSection(sSection)
        If s IsNot Nothing Then
            Dim k As IniSection.IniKey = s.AddKey(sKey)
            If k IsNot Nothing Then
                If sValue = String.Empty Or sValue = Nothing Then
    ...
    Public Class IniSection
        ...
        Private m_keys As Dictionary(Of String, IniKey)

        ' Constuctor so objects are internally managed
        Protected Friend Sub New(ByVal parent As IniFile, ByVal sSection As String)
            m_pIniFile = parent
            m_sSection = sSection
            m_keys = New Dictionary(Of String, IniKey)(StringComparer.InvariantCultureIgnoreCase)
        End Sub
        ...
        Public Class IniKey
            ...
            Public Property Value(Optional ByVal DefaultValue As String = "") As String
                Get
                    If m_sValue = String.Empty Or String.IsNullOrEmpty(m_sValue) Then
                        Return DefaultValue
                    Else
                        Return m_sValue
                    End If
                ...
                Set(ByVal value As String)
                    If value = String.Empty Or String.IsNullOrEmpty(value) Then
            ...
            Public Sub SetValue(ByVal sValue As String)
                m_sValue = sValue
            End Sub
            ' Returns the value of the Key (mod cjcr)
            'Public Function GetValue() As String
            '    Return m_sValue
            'End Function
            Public Function GetValue(Optional ByVal DefaultValue As String = "") As String
                If m_sValue = String.Empty Or String.IsNullOrEmpty(m_sValue) Then
                    Return DefaultValue
                Else
                    Return m_sValue
